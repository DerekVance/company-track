var clientErrors = function(cFirst, cLast, street, city, state, zip, phone){
var arr = [];

  if(cFirst.trim() === '') {
    arr.push('Client first name can not be blank');
  }
  if(cLast.trim() === '') {
    arr.push('Client last name can not be blank');
  }
  if(street.trim() === '') {
    arr.push('Client street can not be blank');
  }
  if(city.trim() === '') {
    arr.push('Client city can not be blank');
  }
  if(state.trim() === '') {
    arr.push('Client state can not be blank');
  }
  if(zip.trim() === '') {
    arr.push('Client zip can not be blank');
  }
  if(phone.trim() === '') {
    arr.push('Phone number can not be blank');
  }
  if(phone.length != 10) {
    arr.push('Phone number must be 10 digits')
  }
  return arr;
};

var employeeErrors = function(eFirst, eLast, phone) {
  var arr = [];
  if(eFirst.trim() === '') {
    arr.push('Employee first name can not be blank');
  }
  if(eLast.trim() === '') {
    arr.push('Employee last name can not be blank');
  }
  if(phone.trim() === '') {
    arr.push('Phone number can not be blank');
  }
  if(phone.length != 10) {
    arr.push('Phone number must be 10 digits')
  }
  return arr;
};

var jobErrors = function(cFirst, cLast, type, eFirst, eLast) {
  var arr = [];
  if(eFirst.trim() === '') {
    arr.push('Employee first name can not be blank');
  }
  if(eLast.trim() === '') {
    arr.push('Employee last name can not be blank');
  }
  if(cFirst.trim() === '') {
    arr.push('Client first name can not be blank');
  }
  if(cLast.trim() === '') {
    arr.push('Client last name can not be blank');
  }
  if(type === 'none') {
    arr.push('Please select a type')
  }
  return arr;
}

module.exports = {
clientErrors: clientErrors,
employeeErrors: employeeErrors,
jobErrors: jobErrors
};
